...
public class NodeController : MonoBehaviour
{
	...
	public void Redraw (NodeData data, Color color)
	{
		_data = data;
		...
		//create a wall		
		if (_data.pos.x > 0 && _data.HasWall (NodeData.DIRECTION_LEFT_IDX)) {
							
			if (_data.pos.y > 0 && _data.HasWall (NodeData.DIRECTION_DOWN_IDX)) 
				_wallInstance = (GameObject)Instantiate (Prefabs.WALL_SOUTH_WEST);
			else
				_wallInstance = (GameObject)Instantiate (Prefabs.WALL_WEST);
							
		} else if (_data.pos.y > 0 && _data.HasWall (NodeData.DIRECTION_DOWN_IDX)) 
		...
		if (_data.HasFlag (NodeData.SPECIALS_EXIT)) 
			_objects.Add (AddObject ((GameObject)Instantiate (Prefabs.EXIT)));
		
										
		if (_data.HasFlag (NodeData.SPECIALS_SPEEDUP_UP)) 
			_objects.Add (AddObject ((GameObject)Instantiate (Prefabs.SPEED_UP)));
						
		if (_data.HasFlag (NodeData.SPECIALS_SPEEDUP_RIGHT)) {
		...
		if (_data.HasFlag (NodeData.SPECIALS_SPEEDUP_DOWN)) {
			GameObject specInstance = (GameObject)Instantiate (Prefabs.SPEED_UP);
			specInstance.transform.eulerAngles = new Vector3 (0, 0, 180);
			_objects.Add (AddObject (specInstance));
		}
						
		if (_data.HasFlag (NodeData.SPECIALS_SPEEDUP_LEFT)) {
			GameObject specInstance = (GameObject)Instantiate (Prefabs.SPEED_UP);
			specInstance.transform.eulerAngles = new Vector3 (0, 0, 90);
			_objects.Add (AddObject (specInstance));
		}
						
		if (_data.HasFlag (NodeData.SPECIALS_ROTATOR_CW)) 
			_objects.Add (AddObject ((GameObject)Instantiate (Prefabs.ROTATOR_CW)));
						
		if (_data.HasFlag (NodeData.SPECIALS_ROTATOR_CCW)) 
			_objects.Add (AddObject ((GameObject)Instantiate (Prefabs.ROTATOR_CCW)));
						
		if (_data.HasFlag (NodeData.SPECIALS_HIDE_WALLS)) 
			_objects.Add (AddObject ((GameObject)Instantiate (Prefabs.HIDE)));
		
		if (_data.HasFlag (NodeData.SPECIALS_SHOW_WALLS)) 
